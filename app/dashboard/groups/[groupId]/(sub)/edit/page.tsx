  ...
  return (
    <Card className="p-8 rounded-2xl shadow-xl bg-white dark:bg-zinc-900 flex flex-col gap-6 border border-primary/20">
      ...
      <p className="text-sm text-default-500 mb-2">
        Update your group details below. Changes are saved instantly for all
        members.
      </p>
          ...
          <span className="text-sm">
            {isPublic ? (
              <span className="flex items-center gap-1 text-success">
                <LockOpenIcon className="h-4 w-4" /> Public group (anyone can
                join)
              </span>
            ) : (
              <span className="flex items-center gap-1 text-warning">
                <LockClosedIcon className="h-4 w-4" /> Private group (invite
                only)
              </span>
            )}
          </span>
